...
function openStatusPanel(c, s, unit){
  ...
  try{
    ...
    // not against the monster standing next to it (v120.284, now that the DM can open this
    ...
    if(near){
      // Adapter must follow the session, not assume Quick Battle (v120.284) - this panel is
...
function renderNotes(c){
  ...
  $('#importBtn').addEventListener('click',()=>$('#importFile').click());
    // Destructive controls bind with onclick, NOT addEventListener (v120.284). onclick is
...
function rollInitiative(){ const s=net.session; const o=[];
  ...
  o.sort((a,b)=>b.roll-a.roll); s.order=o; s.turn=0; s.battle.round=1; s.monsters.forEach(m=>freshMonsterTurn(m)); dmBroadcast(); render(); flashBanner('Initiative rolled'); }
...
function bindBattleCommon(x, moveOpts){
  ...
  // The 3D host gets the SAME move opts the DOM grid got - the v120.284 bug was these two
...
function renderPlayerBattle(c){
  ...
  app.querySelectorAll('[data-cell]').forEach(el=>el.onclick=()=>{
    ...
    const provokers=(s.battle&&s.battle.active&&!(c.battle&&c.battle.disengaged))?s.monsters.filter(mo=>mo.hp>0 && leavesReach(me.x,me.y,x,y,mo.x,mo.y,reachTilesOf(mo),{mover:c, observer:mo}) && !unseenBy(mo,{c})):[];   // Disengage: no opportunity attacks this turn
...
function openAdjacentUseUI(c, s, me){
  ...
  // DM-hosted joined this in v120.284 ("in the dm mode need to be able to have the use and
  ...
  const mode = (typeof QB!=='undefined' && s===QB) ? 'qb'
    : (net && net.role==='player' && s===net.session) ? 'player'
    : (net && net.role==='dm' && s===net.session) ? 'dm' : null;
  const ad = mode==='qb' ? qbAdapter : mode==='player' ? playerNetAdapter
    : mode==='dm' ? sessionAdapter : null;
  const log = mode==='qb' ? qbLog : mode==='player' ? (m=>logChange(c,m))
    : mode==='dm' ? (m=>qbLog(m,s)) : ()=>{};
  // Whose foes? A DM-driven MONSTER is shoving/grappling the party, not its own side, so the
  // adjacency list flips. Everything else (a PC in QB, a player on their own device, or a DM
  // moving a player token) is looking at monsters.
  const actorIsMonster = mode==='dm' && (s.monsters||[]).some(m=>m.id===(me&&me.id));
  const foes = !mode ? []
    : actorIsMonster ? (s.players||[]).filter(p=>(p.hpCur||0)>0 && p.x!=null && gridDist(me.x,me.y,p.x,p.y)<=1)
    : s.monsters.filter(mo=>mo.hp>0 && isHostile(mo) && gridDist(me.x,me.y,mo.x,mo.y)<=1);
  ...
  // unitConds reads BOTH shapes (character .conditions object, monster .conds array), which
  // matters now that the actor here can be a monster.
  ...
  // budget and per-turn flags for whichever kind of unit is acting (v120.284).
  ...
  const setActorFlag = (k,v)=>{ if(mode==='dm' && actorIsMonster) me[k]=v; else if(c.battle) c.battle[k]=v; };
  const grappled=!!mode && actorConds.has('Grappled');
  const canHide=!!mode && !actorConds.has('Hidden'); // maneuverHide itself checks cover/darkness and bails with a banner if ineligible
  ...
  function draw(pick){
    ...
    if(!pick){
      ...
      if(mode && actorHasAction() && !actorConds.has('Dodge')) body+=`<button class="btn block" id="useDodge" style="margin-bottom:8px;text-align:left">🛡 Dodge<small style="display:block;opacity:.75">Action — attacks against you have disadvantage until the start of your next turn</small></button>`;
      if(mode && actorHasAction() && !actorFlag('disengaged')) body+=`<button class="btn block" id="useDisengage" style="margin-bottom:8px;text-align:left">🏃 Disengage<small style="display:block;opacity:.75">Action — your movement won't provoke opportunity attacks this turn</small></button>`;
      ...
      if(mode && actorHasAction() && !actorFlag('readied')) body+=`<button class="btn block" id="useReady" style="margin-bottom:8px;text-align:left">⚡ Ready an attack<small style="display:block;opacity:.75">Action — hold your attack, then release it as a reaction before your next turn</small></button>`;
    ...
    const afterManeuver=()=>{ $('#modalRoot').innerHTML=''; save(); render(); if(mode==='qb') qbCheckEnd(); if(mode==='player') playerHello(); if(mode==='dm') dmBroadcast(); };
    ...
    { const dg=$('#useDodge'); if(dg) dg.onclick=()=>{
      if(!actorHasAction()){ flashBanner('No action left'); return; }
      actorSpendAction();
      // Same Dodge effect Monk's Patient Defense grants — attackAdvantage (rules.js:30) reads the
      // Dodge condition for ANY unit, so a monster gets it through the adapter instead of an
      // effect list it doesn't have. rounds:1 = "until the start of your next turn".
      if(mode==='dm' && actorIsMonster) ad.addCond(me,'Dodge');
      else addEffect(c,'Dodge',{rounds:1, cond:'Dodge', note:'Attack rolls against you have disadvantage.'});
      log('🛡 '+ad.name(me)+' takes the Dodge action');
    ...
    { const di=$('#useDisengage'); if(di) di.onclick=()=>{
      if(!actorHasAction()){ flashBanner('No action left'); return; }
      actorSpendAction();
      setActorFlag('disengaged',true);   // checked at every opportunity-attack provocation point (QB, player-net, DM)
      log('🏃 '+ad.name(me)+' takes the Disengage action — no opportunity attacks this turn');
    ...
    { const rd=$('#useReady'); if(rd) rd.onclick=()=>{
      if(!actorHasAction()){ flashBanner('No action left'); return; }
      actorSpendAction();
      setActorFlag('readied',true);   // cleared at the start of the actor's next turn (freshTurnState / freshMonsterTurn)
      log('⚡ '+ad.name(me)+' readies an attack — held until released as a reaction');
...
function renderDM(){
  ...
  // Move grid for whatever token the DM has selected - monster OR player (v120.284).
  ...
  const dmMoveOpts=(()=>{
    if(!s||!s.battle||!s.battle.active||!net.sel) return {};
    const id=net.sel.slice(1);
    if(net.sel[0]==='m'){ const mo=(s.monsters||[]).find(m=>m.id===id); if(!mo) return {};
      return buildMoveRangeOpts(s,mo,mo.moveLeft||0,(mo.attacksLeft||0)>0,mo.speed||30,monsterFlies(mo)); }
    if(net.sel[0]==='p'){ const pl=(s.players||[]).find(pp=>pp.id===id||pp.cid===id); if(!pl||pl.x==null) return {};
      // The DM places player tokens freely (no budget is deducted), so this is a reach PREVIEW:
      // full speed, no Dash band, rather than a spend-tracking grid like the player's own view.
      const spd=pl.speed||30;
      return buildMoveRangeOpts(s,pl,pl.moveLeft!=null?pl.moveLeft:spd,false,spd,!!pl.flying); }
    return {};
  })();
  app.innerHTML=`
  ...
  ${(s.order&&s.order.length)?(()=>{ const cur=s.order[s.turn]||{}; const curMon=cur.k==='m'?s.monsters.find(m=>m.id===cur.id):null;
    return `<div class="card">
      ...
      ${s.order.map((o,i)=>{ const dead=orderDead(o); return `<div class="listrow" style="${i===s.turn?'background:rgba(77,106,44,.14);border-radius:8px':''}${dead?';opacity:.45':''}"><span style="flex:none;width:22px">${i===s.turn?'▶':''}</span><div class="nm"><b style="${dead?'text-decoration:line-through':''}">${o.k==='m'?'🟥 ':'🟩 '}${esc(o.name)}${dead?' 💀':''}</b></div><input type="number" class="val" data-initedit="${i}" value="${o.roll!=null?o.roll:''}" title="Edit this roll manually" style="max-width:54px;text-align:center;padding:4px 6px"></div>`; }).join('')}
  ...
  <div class="card">
    ...
    ${(()=>{
      ...
      const base=dmMoveOpts;   // hoisted above so the 3D host gets the same grid (v120.284)
      if(asP && asP.x!=null)
  ...
  if(isoView&&iso3dView) syncIso3DHost(s, dmMoveOpts);
  ...
  // Use + Status per unit (v120.284) - the SAME shared panels Quick Battle uses, bound to the
  ...
  app.querySelectorAll('[data-muse]').forEach(el=>el.onclick=()=>{
    ...
    if(mo.x==null){ flashBanner('Place '+mo.name+' on the map first'); return; }
    openAdjacentUseUI(mo, s, mo); });
  app.querySelectorAll('[data-mstat]').forEach(el=>el.onclick=()=>{
    const mo=s.monsters.find(m=>m.id===el.dataset.mstat); if(mo) openStatusPanel(mo, s, mo); });
  app.querySelectorAll('[data-puse]').forEach(el=>el.onclick=()=>{
    const pl=(s.players||[]).find(p=>p.id===el.dataset.puse); if(!pl) return;
    if(pl.x==null){ flashBanner('Place '+pl.name+' on the map first'); return; }
    openAdjacentUseUI(pl, s, pl); });
  app.querySelectorAll('[data-pstat]').forEach(el=>el.onclick=()=>{
    const pl=(s.players||[]).find(p=>p.id===el.dataset.pstat); if(pl) openStatusPanel(pl, s, pl); });
  ...
  app.querySelectorAll('[data-cell]').forEach(el=>el.onclick=()=>{ if(!net.sel) return; const [x,y]=el.dataset.cell.split(',').map(Number);
    ...
    if(net.sel.slice(0,2)==='d:'){ const key=net.sel.slice(2); if(!s.map.decor) s.map.decor={};
      ...
      // and unusable (v120.284 — "i added torch to wall but it didnt add light").
      if(key==='erase'){ delete s.map.decor[dk]; if(s.map.interact) delete s.map.interact[dk]; }
        ...
        s.map.decor[dk]=key;
        if(key==='torch'||key==='torch_unlit'){ s.map.interact=s.map.interact||{};
          s.map.interact[dk]={type:'torch', state:key==='torch'?'lit':'unlit'}; }
        else if(s.map.interact) delete s.map.interact[dk]; }
      syncDecorLight(s, dk, key==='erase'?null:key);
      dmBroadcast(); render(); return; }
    ...
    if(net.sel[0]==='m'){ const mo=s.monsters.find(m=>m.id===net.sel.slice(1)); if(mo){ const fromX=mo.x, fromY=mo.y, wasBattle=s.battle.active, mfly=monsterFlies(mo);
      ...
      if(wasBattle && !mo.disengaged){ const leaving=s.players.filter(p=>(p.hpCur||0)>0 && leavesReach(fromX,fromY,mo.x,mo.y,p.x,p.y,reachTilesOf(p),{mover:mo, observer:p})); leaving.forEach(p=>dmSend(p.id,{t:'oachance',mon:mo.id,name:mo.name,ac:mo.ac})); if(leaving.length) flashBanner('⚔ '+mo.name+' provokes opportunity attack'+(leaving.length>1?'s':'')+' from '+leaving.map(p=>p.name).join(', ')); }
    } }
...
function startQuickBattle(c, crMax, count, mapKey){
  ...
  clearBattleState(c);   // one list, three call sites (v120.284)
...
function qbBeginTurn(){ const o=qbCurrent(); if(!o) return;
  ...
  if(o.k==='m'){ const mo=QB.monsters.find(m=>m.id===o.id); if(!mo||mo.hp<=0){ qbNextTurn(); return; } freshMonsterTurn(mo); render();
...
function qbMovePc(x,y){ const s=QB, pc=s.players[0], c=pc.c, b=c.battle, fly=isFlying(c);
  ...
  function commitMove(opts){
    ...
    const provokers=(b.disengaged)?[]:s.monsters.filter(m=>m.hp>0 && !m.reactionUsed && leavesReach(fromX,fromY,x,y,m.x,m.y,reachTilesOf(m),{mover:c, observer:m}) && !unseenBy(m,QB.players[0]));   // Disengage: movement provokes no opportunity attacks this turn
