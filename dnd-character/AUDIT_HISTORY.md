# 5e Rules Audit - Change History (archive)

**This file is archaeology, not the current ruleset.** It is the append-only, per-version
record of *how* each system came to be. Entries here can describe behaviour that was later
changed or reverted outright - the v114-v118 isometric sequence documents four approaches and
v116 reverted the whole thing - so **never quote this file as current behaviour**.

For what the rules do *today*, read `AUDIT.md`. Come here only to answer "why was it built
this way?", and jump straight to a section by grepping its exact heading from the index below
rather than reading the whole 260 KB.
